...
if __name__ == "__main__":
    ...
    if AES_KEY == "N|ix4sqJ`y#u^8Nrn{8I[rCj]Ih_h0d[" and AES_IV == "3lV7k3}k6c0>n8da" and DB_PASSWORD == "toto":
        print("Voulez vous changer les valeurs par défaut (fortement recommandé) ? (Y/n)", end=" ")
        if input().lower() == "y":
            ...
            CONFIRM = False
            while not CONFIRM:
                if input("êtes-vous sûr du mot de passe pour la base de données (root) ? (Y/n)").lower() == "y":
                    CONFIRM = True
                else:
                    DB_PASSWORD = input("DB_PASSWORD : ")
    ...
    with open(".env", "w") as file:
        ...
        file.write(f"MYSQL_PASSWD={DB_PASSWORD}\n")
        file.close()
    ...
    #scripts = [".\\server.py",".\\client.py", ".\\GUI\\gui.py" ]
    scripts = [".\\server.py" ]
    ...
    os.system(f"python3 -m venv {venvPath}") if os.name != 'nt' else \
        os.system(f"py -m venv {venvPath}")
